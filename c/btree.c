#include <stddef.h>

#include "btree.h"

#include "mem.h"
#include "queue.h"

typedef struct NH_BTree_ NH_BTree;
typedef struct NH_BTreeNode_ NH_BTreeNode;
typedef struct NH_BTreeTraverseInfo_ NH_BTreeTraverseInfo;
typedef void (*NHBTreeInsert) (NH_BTree *tree, NHBTreeCmp cmp, nhptr key,
                               nhptr value);

struct NH_BTree_
{
  struct NH_BTreeNode_
  {
    NH_BTreeNode *left;
    NH_BTreeNode *right;
    nhptr key;
    nhptr value;
    nhint bal;
  } *root;
  NHBTreeCmp cmp;
  NHBTreeInsert insert;
};

struct NH_BTreeTraverseInfo_
{
  nhint level;
  nhint position;
  NHBTreeNode *node;
};

static NH_BTreeNode *
nh_btree_node_rotate_left (NH_BTreeNode *node)
{
  NH_BTreeNode *right = node->right;
  node->right = right->left;
  right->left = node;

  nhint nbal = node->bal;
  nhint rbal = right->bal;

  if (rbal <= 0)
    {
      if (nbal >= 1)
        right->bal = rbal - 1;
      else
        right->bal = nbal + rbal - 2;
      node->bal = nbal - 1;
    }
  else
    {
      if (nbal <= rbal)
        right->bal = nbal - 2;
      else
        right->bal = rbal - 1;
      node->bal = nbal - rbal - 1;
    }

  return right;
}

static NH_BTreeNode *
nh_btree_node_rotate_right (NH_BTreeNode *node)
{
  NH_BTreeNode *left = node->left;
  node->left = left->right;
  left->right = node;

  nhint nbal = node->bal;
  nhint lbal = left->bal;

  if (lbal <= 0)
    {
      if (lbal >= nbal)
        left->bal = lbal + 1;
      else
        left->bal = nbal + 2;
      node->bal = nbal - lbal + 1;
    }
  else
    {
      if (nbal <= -1)
        left->bal = lbal + 1;
      else
        left->bal = nbal + lbal + 2;
      node->bal = nbal + 1;
    }

  return left;
}

static void
nh_btree_insert_node (NH_BTreeNode **place, NH_BTreeNode *node, NHBTreeCmp cmp,
                      nhptr key, nhptr value, nhbool *inserted)
{
  nhint cmp_v = cmp (key, node->key);
  if (cmp_v == 0)
    {
      node->value = value;
      *inserted = FALSE;
      return;
    }
  if (cmp_v < 0)
    {
      if (node->left)
        {
          nhint bal = node->left->bal;
          nh_btree_insert_node (&node->left, node->left, cmp, key, value,
                                inserted);
          if ((bal != node->left->bal) && node->left->bal) node->bal -= 1;
        }
      else
        {
          NH_BTreeNode *new_node = nh_malloc (sizeof (NH_BTreeNode));
          new_node->left = NULL;
          new_node->right = NULL;
          new_node->key = key;
          new_node->value = value;
          new_node->bal = 0;
          node->left = new_node;
          node->bal -= 1;
          *inserted = TRUE;
        }
      return;
    }
  if (node->right)
    {
      nhint bal = node->right->bal;
      nh_btree_insert_node (&node->right, node->right, cmp, key, value,
                            inserted);
      if ((bal != node->right->bal) && node->right->bal) node->bal += 1;
    }
  else
    {
      NH_BTreeNode *new_node = nh_malloc (sizeof (NH_BTreeNode));
      new_node->left = NULL;
      new_node->right = NULL;
      new_node->key = key;
      new_node->value = value;
      new_node->bal = 0;
      node->right = new_node;
      node->bal += 1;
      *inserted = TRUE;
    }
  if (*inserted)
    {
      if (node->bal < -1)
        {
          if (node->left->bal > 0)
            node->left = nh_btree_node_rotate_left (node->left);
          *place = nh_btree_node_rotate_right (node);
        }
      else if (node->bal > 1)
        {
          if (node->right->bal < 0)
            node->right = nh_btree_node_rotate_right (node->right);
          *place = nh_btree_node_rotate_left (node);
        }
    }
}

static void
nh_btree_insert_start (NH_BTree *tree, NHBTreeCmp cmp, nhptr key, nhptr value)
{
  NH_BTreeNode *root = tree->root;
  nhint cmp_v = cmp (key, root->key);
  nhbool inserted = FALSE;
  if (cmp_v == 0)
    {
      root->value = value;
      return;
    }
  if (cmp_v < 0)
    {
      if (root->left)
        {
          nh_btree_insert_node (&root->left, root->left, cmp, key, value,
                                &inserted);
        }
      else
        {
          NH_BTreeNode *new_node = nh_malloc (sizeof (NH_BTreeNode));
          new_node->left = NULL;
          new_node->right = NULL;
          new_node->key = key;
          new_node->value = value;
          new_node->bal = root->bal + 1;
          root->left = new_node;
        }
      return;
    }
  if (root->right)
    {
      nh_btree_insert_node (&root->right, root->right, cmp, key, value,
                            &inserted);
    }
  else
    {
      NH_BTreeNode *new_node = nh_malloc (sizeof (NH_BTreeNode));
      new_node->left = NULL;
      new_node->right = NULL;
      new_node->key = key;
      new_node->value = value;
      new_node->bal = root->bal + 1;
      root->right = new_node;
    }
}

static void
nh_btree_insert_root (NH_BTree *tree, NHBTreeCmp cmp, nhptr key, nhptr value)
{
  NH_BTreeNode *node = nh_malloc (sizeof (NH_BTreeNode));
  node->left = NULL;
  node->right = NULL;
  node->key = key;
  node->value = value;
  node->bal = 0;
  tree->root = node;
  tree->insert = nh_btree_insert_start;
}

NHBTree *
nh_btree_new (NHBTreeCmp cmp)
{
  NH_BTree *tree = nh_malloc (sizeof (NH_BTree));
  tree->root = NULL;
  tree->cmp = cmp;
  tree->insert = nh_btree_insert_root;
  return (NHBTree *)tree;
}

static void
nb_btree_node_free (NHBTreeNode *node)
{
  if (!node) return;
  nb_btree_node_free (node->left);
  nb_btree_node_free (node->right);
  nh_free (node);
}

void
nh_btree_free (NHBTree *tree)
{
  nb_btree_node_free (tree->root);
  nh_free (tree);
}

void
nh_btree_insert (NHBTree *tree_, nhptr key, nhptr value)
{
  NH_BTree *tree = (NH_BTree *)tree_;
  tree->insert (tree, tree->cmp, key, value);
}

static nhbool
nh_btree_node_preorder_traverse (NH_BTreeNode *node,
                                 NHBTreeTraverser traverser)
{
  if (traverser (node->key, node->value)) return TRUE;
  if (node->left)
    if (nh_btree_node_preorder_traverse (node->left, traverser)) return TRUE;
  if (node->right)
    if (nh_btree_node_preorder_traverse (node->right, traverser)) return TRUE;
  return FALSE;
}

void
nh_btree_preorder_traverse (NHBTree *tree, NHBTreeTraverser traverser)
{
  nh_btree_node_preorder_traverse (((NH_BTree *)tree)->root, traverser);
}

static nhbool
nh_btree_node_inorder_traverse (NH_BTreeNode *node, NHBTreeTraverser traverser)
{
  if (node->left)
    if (nh_btree_node_inorder_traverse (node->left, traverser)) return TRUE;
  if (traverser (node->key, node->value)) return TRUE;
  if (node->right)
    if (nh_btree_node_inorder_traverse (node->right, traverser)) return TRUE;
  return FALSE;
}

void
nh_btree_inorder_traverse (NHBTree *tree, NHBTreeTraverser traverser)
{
  nh_btree_node_inorder_traverse (((NH_BTree *)tree)->root, traverser);
}

static nhbool
nh_btree_node_postorder_traverse (NH_BTreeNode *node,
                                  NHBTreeTraverser traverser)
{
  if (node->left)
    if (nh_btree_node_postorder_traverse (node->left, traverser)) return TRUE;
  if (node->right)
    if (nh_btree_node_postorder_traverse (node->right, traverser)) return TRUE;
  if (traverser (node->key, node->value)) return TRUE;
  return FALSE;
}

void
nh_btree_postorder_traverse (NHBTree *tree, NHBTreeTraverser traverser)
{
  nh_btree_node_postorder_traverse (((NH_BTree *)tree)->root, traverser);
}

void
nh_btree_levelorder_traverse (NHBTree *tree, NHBTreeTraverser traverser)
{
  NHQueue *queue = nh_queue_new ();
  nh_queue_push (queue, tree->root);

  while (!nh_queue_empty (queue))
    {
      NH_BTreeNode *node = nh_queue_pop (queue);
      if (traverser (node->key, node->value)) return;
      if (node->left) nh_queue_push (queue, node->left);
      if (node->right) nh_queue_push (queue, node->right);
    }

  nh_queue_free (queue);
}

void
nh_btree_levelorder_info_traverse (NHBTree *tree,
                                   NHBTreeInfoTraverser traverser)
{
  NHQueue *queue = nh_queue_new ();
  NH_BTreeTraverseInfo infos[50];
  nhint infos_count = 0;

  infos[0] = (NH_BTreeTraverseInfo){ .level = 0,
                                     .position = 0,
                                     .node = tree->root };
  infos_count++;

  nh_queue_push (queue, &infos[0]);

  while (!nh_queue_empty (queue))
    {
      NH_BTreeTraverseInfo *info = nh_queue_pop (queue);
      NHBTreeNode *node = info->node;
      if (traverser (node->key, node->value, (NHBTreeTraverseInfo *)info))
        return;
      if (node->left)
        {
          infos[infos_count]
              = (NH_BTreeTraverseInfo){ .level = info->level + 1,
                                        .position = 2 * info->position,
                                        .node = node->left };
          nh_queue_push (queue, &infos[infos_count++]);
        }
      if (node->right)
        {
          infos[infos_count]
              = (NH_BTreeTraverseInfo){ .level = info->level + 1,
                                        .position = 2 * info->position + 1,
                                        .node = node->right };
          nh_queue_push (queue, &infos[infos_count++]);
        }
    }

  nh_queue_free (queue);
}
