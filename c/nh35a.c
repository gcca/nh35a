...
void _heapq_siftdown(struct heapq_t *heapq, size_t startpos, size_t pos) {
...
void _heapq_siftup(struct heapq_t *heapq, size_t pos) {
...
void _heapq_traverse_ld(struct heapq_t *heapq, size_t pos) {
  if (pos >= heapq->size)
    return;
  size_t cpos = 2 * pos + 1;
...
void heapq_traverse_ld(struct heapq_t *heapq) { _heapq_traverse_ld(heapq, 0); }
