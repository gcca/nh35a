...
nh_stack_new ()
{
  ...
  NHChunk *chunk
      = nh_chunk_new (sizeof (NHStackNode), sizeof (NHStackNode) * 1024);
  stack->allocator = nh_allocator_new (chunk);

  stack->root = nh_allocator_alloc (stack->allocator);
...
nh_stack_free (NHStack *stack_)
{
  ...
  nh_allocator_free (stack->allocator);
  nh_free (stack);
}
...
nh_stack_push (NHStack *stack_, nhptr data)
{
  ...
  NHStackNode *node = nh_allocator_alloc (stack->allocator);
...
nh_stack_pop (NHStack *stack_)
{
  ...
  nh_allocator_dealloc (stack->allocator, node);
